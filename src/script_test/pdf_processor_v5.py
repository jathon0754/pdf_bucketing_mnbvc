...
# Author: Jathon
# Date: 2025/12/02
# Description: 高性能 PDF 处理器（只读取元信息，极快）
...
def process_single_pdf(pdf_path_str):
    ...
    try:
        ...
        if doc.is_encrypted:
            ...
            info['page_count'] = doc.page_count  # 快速读取页数
            info['can_open'] = True
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="极致高性能 PDF 扫描器 (只读元信息)")
